...
let AngularFireLiteApp = /** @class */ (function () {
...
let AngularFireLiteAuth = /** @class */ (function () {
    ...
    AngularFireLiteAuth.prototype.isAuthenticated = function () {
        let IS_AUTHENTICATED = new Subject();
...
let AngularFireLiteDatabase = /** @class */ (function () {
    ...
    AngularFireLiteDatabase.prototype.read = function (ref) {
        let dataStateKey = makeStateKey(ref);
        if (this.server) {
    ...
    AngularFireLiteDatabase.prototype.SRH = function (ref, DSK) {
        ...
        return this.http.get("https://" + this.config.projectId + ".firebaseio.com/" + ref + ".json")
            ...
            if (!!payload && typeof payload === 'object') {
                let result = Object.keys(payload).map(function (key) {
                    return [payload[key]];
                });
    ...
    AngularFireLiteDatabase.prototype.BRH = function (ref, event, DSK) {
        if (this.browser) {
            let SSRedValue = this.state.get(DSK, []);
            let DATA_1 = new BehaviorSubject(SSRedValue);
            this.ref(ref).on(event, function (snapshot) {
                if (!!snapshot.val() && typeof snapshot.val() === 'object') {
                    let result = Object.keys(snapshot.val()).map(function (key) {
    ...
    AngularFireLiteDatabase.prototype.query = function (ref) {
        let PID = this.platformId;
        let state = this.state;
        let db = this.database;
        let http = this.http;
        let config = this.config;
        let SQH = function (REF, FSQ, DSK) {
        ...
        return {
            ...
            on: function (event) {
                ...
                if (isPlatformBrowser(PID)) {
                    let SSRedValue = state.get(dataStateKey, []);
                    let VALUE = new BehaviorSubject(SSRedValue);
...
let AngularFireLiteFirestore = /** @class */ (function () {
    ...
    AngularFireLiteFirestore.prototype.read = function (ref) {
                ...
                if (slashes % 2 !== 0) {
                    ...
                    res.documents.forEach(function (doc) {
                        ...
                        Object.keys(doc.fields)
                            .forEach(function (key) {
                            for (let keyValue in doc.fields[key]) {
                                if (keyValue) {
        ...
        if (this.browser) {
            let data_1 = [];
            let SSRedValue = this.state.get(dataStateKey, []);
            let DATA_2 = new BehaviorSubject(SSRedValue);
            if (slashes % 2 === 0) {
    ...
    AngularFireLiteFirestore.prototype.query = function (ref) {
        ...
        return {
            ...
            on: function () {
                let ONDSK = makeStateKey(ref + ':query');
                if (isPlatformServer(PID)) {
                    return SQHFS(ONDSK);
                }
                if (isPlatformBrowser(PID)) {
                    let SSRedValue = state.get(ONDSK, []);
                    let VALUE_1 = new BehaviorSubject(SSRedValue);
            ...
            get: function () {
                let GETDSK = makeStateKey(ref + ':query');
                if (isPlatformServer(PID)) {
                    return SQHFS(GETDSK);
                }
                if (isPlatformBrowser(PID)) {
                    let data_2 = [];
                    let SSRedValue = state.get(GETDSK, []);
                    let VALUE_2 = new BehaviorSubject(SSRedValue);
    ...
    AngularFireLiteFirestore.prototype.transaction = function () {
        if (this.browser) {
            let fs_1 = this.firestore;
            let transactionToRun_1;
            let readCount_1 = 0;
            let transactions_1 = {
                get: function (ref) {
    ...
    AngularFireLiteFirestore.prototype.batch = function () {
        if (this.browser) {
            let fs_2 = this.firestore;
            let b_1 = this.firestore.batch();
            return {
                set: function (ref, data) {
...
let AngularFireLiteStorage = /** @class */ (function () {
...
let AngularFireLiteMessaging = /** @class */ (function () {
    ...
    AngularFireLiteMessaging.prototype.tokenRefresh = function () {
        let _this = this;
        if (this.browser) {
            let REFRESH_TOKEN_1 = new Subject();
...
let FirebaseAppConfig = /** @class */ (function () {
    function FirebaseAppConfig() {
    }
    return FirebaseAppConfig;
}());
let AngularFireLite = /** @class */ (function () {
